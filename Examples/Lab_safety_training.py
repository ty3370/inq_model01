...
prompt_scene = (
    ...
    "사용자가 구체적으로 어떤 구역을 조사한다고 언급했을 때에만 응답한다. 절대 구체적 증거를 선제적으로 제시하지 않는다. 예를 들어 사용자가 실험실을 전체적으로 둘러본다거나 의심스러운 부분을 찾아본다거나 조사할 부분을 찾아본다는 등의 멘트를 입력한 경우, '사고가 발생한 모둠의 테이블과 천장의 화재 흔적 외에는 평범한 실험실의 모습이다.' 정도의 일반적인 답변만 제공한다."
)
