...
initial_prompt = (
    ...
    "다음은 가설에 관한 채점 기준입니다: 1. 조작 변인이 있는가? 2. 종속 변인이 있는가? 3. 조작 변인과 종속 변인의 관계가 제시되었는가(A가 ~할수록 B가 ~하다)?"
    "다음은 실험 과정에 관한 채점 기준입니다: 4. 조작변인 조절을 위한 구체적 조건을 제시하였는가? 5. 일정하게 해야 할 변인을 통제하기 위한 구체적 언급이 있는가? 6. 실제로 실험에 사용될 준비물을 제시하였는가? 7. 반복 실험에 관한 언급이 있는가?"
    "평가 결과를 제시할 때 가독성을 고려해 줄바꿈 하세요."
