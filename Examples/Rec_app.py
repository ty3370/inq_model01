...
with tab4:
    ...
    st.info("학생 활동 자료를 이미지나 PDF로 모아두신 선생님들을 위한 탭입니다. 학생 활동지, 메모, 서류 등의 이미지 파일(PNG, JPG)이나 PDF 파일을 업로드하면 내용을 텍스트로 변환합니다. 텍스트로 변환된 내용을 생기부 생성 탭에서 '학생의 개별 활동 자료'로 입력하시면 됩니다.")
