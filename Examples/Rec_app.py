...
initial_prompt = (
    ...
    "AR은 증강현실로 대체"
    "AI는 인공지능으로 대체"
    "HTML 등은 하이퍼텍스트 마크업 언어, 웹 페이지 제작 언어로 대체"
    "CSS는 스타일 시트 언어로 대체"
...
activity_prompt = (
    ...
    "자율활동 내용과 날짜를 바탕으로 입력 내용을 최대한 풍부하게 생성하세요."
    "문장을 시작할 때 절대로 '이 학생은' 또는 '학생은'이라는 말로 시작하면 안 됩니다."
...
with tab1:
    ...
    with st.form(key='form_tab1', clear_on_submit=False):
        ...
        act_student_data = st.text_area("학생의 개별 활동 자료:", placeholder="예시: 온라인 예절의 중요성 발표", key="act_student_data")
...
with tab2:
    ...
    with st.form(key='form_tab2', clear_on_submit=False):
        ...
        sub_student_data = st.text_area("학생의 개별 활동 자료:", placeholder="예시: 안전모의 원리 발표", key="sub_student_data")
